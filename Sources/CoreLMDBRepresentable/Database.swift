...
extension Database {
    ...
    public func delete<Key: RawBufferRepresentable>(atKey key: Key, in transaction: Transaction) throws {
        try key.withUnsafeBytes { key in
            try delete(atKey: key, withDuplicateValue: nil, in: transaction)
        }
    }
    ...
    /// - Throws: An `LMDBError` if the operation fails.
    @inlinable @inline(__always)
    public func delete<Key: RawBufferRepresentable, Value: RawBufferRepresentable>(atKey key: Key, withDuplicateValue value: Value, in transaction: Transaction) throws {
        try key.withUnsafeBytes { key in
            try value.withUnsafeBytes { value in
                try delete(atKey: key, withDuplicateValue: value, in: transaction)
            }
        }
    }
}
